...
// 2. Database Connection (Verified TiDB Cloud Settings)
...
// 3. Auth Middleware (Simplified for Demo)
const authenticate = (req, res, next) => {
    const authHeader = req.headers['authorization'];
    if (!authHeader) return res.status(401).json({ message: "No token provided" });
    req.studentId = 1; // Assuming MAT001 for demo purposes
    next();
};
...
app.get('/api/election', authenticate, (req, res) => {
    ...
    db.query(query, (err, results) => {
        if (err) return res.status(500).json({ message: "Error fetching election" });
        if (!results || results.length === 0) return res.json({ positions: [] });

        const election = { id: results[0].e_id, name: results[0].e_name, status: results[0].e_status };
        const positionsMap = {};
        ...
        res.json({ election, positions: Object.values(positionsMap) });
    });
});

// 6. Get Voting Status
app.get('/api/status', authenticate, (req, res) => {
    db.query('SELECT has_voted FROM students WHERE id = ?', [req.studentId], (err, results) => {
        if (err) return res.status(500).json({ message: "Error" });
        res.json({ has_voted: results[0].has_voted === 1, election_status: "open" });
    });
});
...
app.post('/api/votes', authenticate, (req, res) => {
    ...
    db.query('SELECT has_voted FROM students WHERE id = ?', [studentId], (err, results) => {
        ...
        db.query('INSERT INTO votes (student_id, election_id, position_id, candidate_id) VALUES ?', [voteValues], (err) => {
            if (err) return res.status(500).json({ message: "Voting failed" });
    ...
    db.query(query, (err, results) => {
        ...
        results.forEach(row => {
            ...
            resultsMap[row.position_name].results.push({
                ...
                percentage: 0 // Logic can be added here
            });
        });

        res.json({ election: { name: "2026 SUG Results" }, results: Object.values(resultsMap) });
    });
});
